...
class ZendX_JQuery_View_Helper_JQuery_Container
{
    ...
    protected function _renderScriptTags()
    {
        ...
        if( ($this->getRenderMode() & ZendX_JQuery::RENDER_LIBRARY) > 0) {
            $source = 'http://informes-gerenciales/js/jquery-1.7.1.js';//$this->_getJQueryLibraryPath();
   ...
   /* protected function _getJQueryLibraryBaseCdnUri()
    ...
    {
        if($this->useUiCdn()) {
              $uiPath = 'http://informes-gerenciales/js/jquery-ui-1.8.17.custom.min.js';//$this->_getJQueryLibraryBaseCdnUri();
            /*$uiPath = $baseUri.
